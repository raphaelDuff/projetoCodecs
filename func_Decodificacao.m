...
T = dctmtx(8);
dct_inv_Y =  @(block_struct) ((T'*block_struct.data*T).*(Q1*S));
dct_inv_PbPr = @(block_struct) ((T'*block_struct.data*T).*(Q2*S));

yfinal= blockproc(Yq, [8 8], dct_inv_Y);
Pbfinal=blockproc(Pbq, [8 8], dct_inv_PbPr);
Prfinal=blockproc(Prq, [8 8], dct_inv_PbPr);
